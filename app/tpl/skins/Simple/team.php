	...
	<body>
		<nav>
			<ul>
				...
				<li onclick="location.href='{url}/team'"><strong>Team</strong></li>
		...
		<?php
			...
			foreach($users as $user):
				$online = sprintf('<span style="display: inline-block; background: url({url}/app/tpl/skins/Habbo/img/%s.gif) left center no-repeat; width: 40px; height: 16px;"></span>', ($user['online'] == 1) ? 'online' : 'offline');
                                ...
                                <?php
									...
									if(!$badges || empty($badges))
									{
										...
										foreach($badges as $badge)
											echo sprintf('<span style="display: inline-block; background: url(%s/c_images/badges/%s.gif) center no-repeat; margin 0 14px; width: 50px; height: 40px;"></span>', $core::getHotelSwfFolder(), $badge['badge_id']);
									}
								?>
                            </td>
                        </tr>
